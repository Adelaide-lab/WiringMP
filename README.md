# WiringMP
This is a GPIO access library for NXP imx6.Soc. It is based on the WiringMP for Nano Pi which is based on original WiringPi for Raspberry Pi.
  
# Installation

## Install WiringMP 
Log into your nano board via SSH, open a terminal and install the WiringWP library by running the following commands:
```
git clone 
cd WiringMP/
chmod 755 build
...
# Verify WiringMP
The WiringMP library contains a set of gpio commands. Users can use them to access the GPIO pins on a nano board. You can verify your WiringMP by running the following command:
```
gpio readall
```
...
# PWM Code Sample
Cause our mother board's PWM pin is not strenched out, so there would be no pwm examples for this.But I will start my personal board very soon.
